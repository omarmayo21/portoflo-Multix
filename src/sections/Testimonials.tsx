...
export const Testimonials: React.FC = () => {
  ...
  const { testimonials: sanityTestimonials } = useSanity();
  const [currentIndex, setCurrentIndex] = useState(0);

  const defaultTestimonials = t('testimonials.items') as any[];

  const testimonials =
    sanityTestimonials && sanityTestimonials.length > 0
      ? sanityTestimonials.map((item) => ({
          id: item._id,
          name: item.name?.[language] || item.name?.en || '',
          role: item.role?.[language] || item.role?.en || '',
          company: item.company || '',
          content: item.review?.[language] || item.review?.en || '',
          avatar: item.avatar ? urlFor(item.avatar).width(120).url() : 'https://images.unsplash.com/photo-1534528741775-53994a69daeb',
